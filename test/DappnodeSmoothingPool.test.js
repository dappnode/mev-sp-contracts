...
describe("DappnodeSmoothingPool test", () => {
  ...
  const subscriptionCollateral = BigInt(ethers.parseEther("0.01"));
  const poolFee = 1000;
  ...
  beforeEach("Deploy contract", async () => {
    ...
    const dappnodeSmoothingPoolFactory = await ethers.getContractFactory(
  ...
  it("should check the initialize", async () => {
    ...
    expect(await dappnodeSmoothingPool.governance()).to.be.equal(
      governance.address
    );
    expect(await dappnodeSmoothingPool.subscriptionCollateral()).to.be.equal(
      subscriptionCollateral
    );
    expect(await dappnodeSmoothingPool.poolFee()).to.be.equal(poolFee);
    expect(await dappnodeSmoothingPool.poolFeeRecipient()).to.be.equal(
      deployer.address
    );
    expect(await dappnodeSmoothingPool.checkpointSlotSize()).to.be.equal(
      checkpointSlotSize
    );
    ...
    expect(await dappnodeSmoothingPool.deploymentBlockNumber()).to.be.equal(
      (await dappnodeSmoothingPool.deploymentTransaction().wait()).blockNumber
    );
  });
  ...
  it("should check the initialize", async () => {
    ...
    const smoothingTestContractInit =
      await dappnodeSmoothingPoolFactory.deploy();
    await smoothingTestContractInit.waitForDeployment();

    await expect(
      smoothingTestContractInit.initialize(
        governance.address,
        subscriptionCollateral,
        10001, // pool fee
        deployer.address, // pool fee recipient
  ...
  it("should check the fallback function", async () => {
    const donationValue = ethers.parseEther("1");
    await expect(
      donator.sendTransaction({
        to: dappnodeSmoothingPool.target,
        value: donationValue,
      })
    )
      .to.emit(dappnodeSmoothingPool, "EtherReceived")
      .withArgs(donator.address, donationValue);
  ...
  it("Should suscribe validator and unsubscribe", async () => {
    ...
    await expect(
      dappnodeSmoothingPool.subscribeValidator(validatorID)
    ).to.be.revertedWith(
    ...
    await expect(
      dappnodeSmoothingPool.subscribeValidator(validatorID, {
        value: subscriptionCollateral + 1n,
      })
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::subscribeValidator: msg.value does not equal subscription collateral"
    );

    const initialSmoothingPoolEther = await ethers.provider.getBalance(
      dappnodeSmoothingPool.target
    );

    await expect(
      dappnodeSmoothingPool.subscribeValidator(validatorID, {
        value: subscriptionCollateral,
      })
    )
    ...
    expect(
      await ethers.provider.getBalance(dappnodeSmoothingPool.target)
    ).to.be.equal(initialSmoothingPoolEther + subscriptionCollateral);
  ...
  it("Should ban and unbann a validator and unsubscribe", async () => {
    ...
    // ban validators
    await expect(
      dappnodeSmoothingPool.connect(deployer).banValidators([validatorID])
    ).to.be.revertedWith(
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .banValidators([validatorID, validatorID2, validatorID3])
    )
    ...
    // unabn validators
    await expect(
      dappnodeSmoothingPool.connect(deployer).unbanValidators([validatorID])
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::onlyGovernance: Only governance"
    );
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .unbanValidators([validatorID, validatorID2, validatorID3])
    )
  ...
  it("Should suscribe multiple validators", async () => {
    ...
    // Check subscribeValidator
    await expect(
      dappnodeSmoothingPool.subscribeValidators([
        validatorID,
        validatorID2,
        validatorID3,
      ])
    ...
    await expect(
      dappnodeSmoothingPool.subscribeValidators(
        ...
        { value: subscriptionCollateral * 2n }
      )
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::subscribeValidator: msg.value does not equal subscription collateral"
    );
    ...
    await expect(
      dappnodeSmoothingPool.subscribeValidators(
        [validatorID, validatorID2, validatorID3],
        { value: subscriptionCollateral * 3n }
      )
    )
      .to.emit(dappnodeSmoothingPool, "SubscribeValidator")
      .withArgs(deployer.address, subscriptionCollateral, validatorID)
    ...
    expect(
      await ethers.provider.getBalance(dappnodeSmoothingPool.target)
    ).to.be.equal(initialSmoothingPoolEther + subscriptionCollateral * 3n);
  });

  it("should check governance methods", async () => {
    // Add oracle members
    expect(
      await dappnodeSmoothingPool.addressToVotedReportHash(
        oracleMember1.address
      )
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(deployer)
        .addOracleMember(oracleMember1.address)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::onlyGovernance: Only governance"
    );
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .addOracleMember(oracleMember2.address)
    )
      .to.emit(dappnodeSmoothingPool, "AddOracleMember")
    ...
    expect(
      await dappnodeSmoothingPool.addressToVotedReportHash(
        oracleMember2.address
      )
    ).to.be.equal(await dappnodeSmoothingPool.INITIAL_REPORT_HASH());

    const oracleMemberIndex1 = 0;
    expect(
      await dappnodeSmoothingPool.getOracleMemberIndex(oracleMember1.address)
    ).to.be.equal(oracleMemberIndex1);
    expect(
      await dappnodeSmoothingPool.getOracleMemberIndex(oracleMember2.address)
    ).to.be.equal(1);

    expect(await dappnodeSmoothingPool.getOracleMembersCount()).to.be.equal(2);

    expect(await dappnodeSmoothingPool.getAllOracleMembers()).to.deep.equal([
      oracleMember1.address,
      oracleMember2.address,
    ]);
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(deployer)
        .removeOracleMember(oracleMember1.address, oracleMemberIndex1)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::onlyGovernance: Only governance"
    );
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .removeOracleMember(deployer.address, oracleMemberIndex1)
    ).to.be.revertedWith(
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .removeOracleMember(oracleMember2.address, oracleMemberIndex1)
    ).to.be.revertedWith(
    ...
    await expect(
      dappnodeSmoothingPool.getOracleMemberIndex(oracleMember1.address)
    ).to.be.revertedWith(
    ...
    expect(await dappnodeSmoothingPool.getOracleMembersCount()).to.be.equal(1);
    ...
    expect(await dappnodeSmoothingPool.quorum()).to.be.equal(quorum);
    await expect(
      dappnodeSmoothingPool.connect(deployer).updateQuorum(newQuorum)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::onlyGovernance: Only governance"
    );

    await expect(
      dappnodeSmoothingPool.connect(governance).updateQuorum(0)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::updateQuorum: Quorum cannot be 0"
    );

    await expect(
      dappnodeSmoothingPool.connect(governance).updateQuorum(newQuorum)
    )
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(deployer)
        .transferGovernance(deployer.address)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::onlyGovernance: Only governance"
    );

    expect(await dappnodeSmoothingPool.pendingGovernance()).to.be.equal(
      ethers.ZeroAddress
    );
    await expect(
      dappnodeSmoothingPool
    ...
    await expect(
      dappnodeSmoothingPool.connect(governance).acceptGovernance()
    ).to.be.revertedWith(
  ...
  it("should check owner methods", async () => {
    ...
    await expect(
      dappnodeSmoothingPool.connect(governance).initSmoothingPool(initSlot)
    ).to.be.revertedWith("Ownable: caller is not the owner");

    await expect(
      dappnodeSmoothingPool.connect(deployer).initSmoothingPool(0)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::initSmoothingPool: Cannot initialize to slot 0"
    );

    await expect(
      dappnodeSmoothingPool.connect(deployer).initSmoothingPool(initSlot)
    )
      .to.emit(dappnodeSmoothingPool, "InitSmoothingPool")
      .withArgs(initSlot);

    await expect(
      dappnodeSmoothingPool.connect(deployer).initSmoothingPool(2)
    ).to.be.revertedWith(
    ...
    const newCollateral = subscriptionCollateral * 2n;
    await expect(
      dappnodeSmoothingPool.connect(governance).updateCollateral(newCollateral)
    ).to.be.revertedWith("Ownable: caller is not the owner");

    await expect(
      dappnodeSmoothingPool.connect(deployer).updateCollateral(newCollateral)
    )
    ...
    await expect(
      dappnodeSmoothingPool.connect(governance).updatePoolFee(newPoolFee)
    ).to.be.revertedWith("Ownable: caller is not the owner");

    await expect(
      dappnodeSmoothingPool.connect(deployer).updatePoolFee(10001)
    ).to.be.revertedWith(
      "DappnodeSmoothingPool::updatePoolFee: Pool fee cannot be greater than 100%"
    );

    await expect(
      dappnodeSmoothingPool.connect(deployer).updatePoolFee(newPoolFee)
    )
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .updatePoolFeeRecipient(poolFeeRecipient)
    ).to.be.revertedWith("Ownable: caller is not the owner");
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .updateCheckpointSlotSize(newCheckpointSlotSize)
    ).to.be.revertedWith("Ownable: caller is not the owner");
    ...
    expect(await dappnodeSmoothingPool.checkpointSlotSize()).to.be.equal(
      newCheckpointSlotSize
    );
  });

  it("should check oracle methods with quorum 1", async () => {
    // Check update updateRewardsRoot root
    expect(await dappnodeSmoothingPool.rewardsRoot()).to.be.equal(
      ethers.ZeroHash
    );

    const valuesRewards = [
      [deployer.address, ethers.parseEther("1")],
      [validator2.address, ethers.parseEther("1")],
    ];
    const leafsRewards = valuesRewards.map((rewardLeaf) =>
      ethers.solidityPackedKeccak256(["address", "uint256"], rewardLeaf)
    );
    const rewardsMerkleTree = new MerkleTree(leafsRewards, ethers.keccak256, {
      sortPairs: true,
      duplicateOdd: true,
    });
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .submitReport(slotNumber, rewardsMerkleTree.getHexRoot())
    ).to.be.revertedWith(
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(deployer)
        .initSmoothingPool(checkpointSlotSize)
    )
      .to.emit(dappnodeSmoothingPool, "InitSmoothingPool")
      .withArgs(checkpointSlotSize);
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(oracleMember1)
        .submitReport(0, rewardsMerkleTree.getHexRoot())
    ).to.be.revertedWith(
  ...
  it("should check oracle methods with quorum 2", async () => {
    ...
    const votedReportHash = await dappnodeSmoothingPool.getReportHash(
      slotNumber,
      rewardsMerkleTree.getHexRoot()
    );
    ...
    const currentVotedReport = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash
    );
    ...
    let consolidatedReport = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash
    );
    ...
    const votedReportHash2 = await dappnodeSmoothingPool.getReportHash(
      slotNumber + checkpointSlotSize,
      rewardsMerkleTree.getHexRoot()
    );
    expect(
    ...
    ).to.be.equal(votedReportHash2);
    let currentVotedReport2 = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash2
    );
    expect(currentVotedReport2.slot).to.be.equal(
      slotNumber + checkpointSlotSize
    );
     ...
     * remove oracle 1
     */
    let oracleMemberIndex = await dappnodeSmoothingPool.getOracleMemberIndex(
      oracleMember1.address
    );
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .removeOracleMember(oracleMember1.address, oracleMemberIndex)
    )
      .to.emit(dappnodeSmoothingPool, "RemoveOracleMember")
      .withArgs(oracleMember1.address);
    expect(
    ...
    consolidatedReport = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash
    );
    expect(consolidatedReport.slot).to.be.equal(0);
    expect(consolidatedReport.votes).to.be.equal(0);
    ...
    // Remove oracle 2
    oracleMemberIndex = await dappnodeSmoothingPool.getOracleMemberIndex(
      oracleMember2.address
    );
    await expect(
      dappnodeSmoothingPool
        .connect(governance)
        .removeOracleMember(oracleMember2.address, oracleMemberIndex)
    )
      .to.emit(dappnodeSmoothingPool, "RemoveOracleMember")
      .withArgs(oracleMember2.address);
    expect(
    ...
    currentVotedReport2 = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash2
    );
    expect(currentVotedReport2.slot).to.be.equal(
      slotNumber + checkpointSlotSize
  ...
  it("should check change report", async () => {
    ...
    let currentVotedReport = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash
    );
    expect(currentVotedReport.slot).to.be.equal(slotNumber);
    expect(currentVotedReport.votes).to.be.equal(currentVotes);
    ...
    currentVotedReport = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash
    );
    expect(currentVotedReport.slot).to.be.equal(slotNumber);
    expect(currentVotedReport.votes).to.be.equal(currentVotes);
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(oracleMember1)
        .submitReport(slotNumber, ethers.ZeroHash)
    )
      .to.emit(dappnodeSmoothingPool, "SubmitReport")
      .withArgs(slotNumber, ethers.ZeroHash, oracleMember1.address);

    expect(await dappnodeSmoothingPool.rewardsRoot()).to.be.equal(
      ethers.ZeroHash
    );
    const newVotedReportHash = await dappnodeSmoothingPool.getReportHash(
      slotNumber,
      ethers.ZeroHash
    );
    expect(
    ...
    const previousVotedReport = await dappnodeSmoothingPool.reportHashToReport(
      votedReportHash
    );
    ...
    await expect(
      ...
      .to.emit(dappnodeSmoothingPool, "SubmitReport")
      .withArgs(slotNumber, ethers.ZeroHash, oracleMember2.address)
      .to.emit(dappnodeSmoothingPool, "ReportConsolidated")
      .withArgs(slotNumber, ethers.ZeroHash);
    ...
    await expect(
      dappnodeSmoothingPool
        .connect(oracleMember1)
        .submitReport(slotNumber + checkpointSlotSize, ethers.ZeroHash)
    )
      .to.emit(dappnodeSmoothingPool, "SubmitReport")
      .withArgs(
        slotNumber + checkpointSlotSize,
  ...
  it("Should claimRewards", async () => {
    const availableBalanceValidator1 = ethers.parseEther("10");
    const availableBalanceValidator2 = ethers.parseEther("1");
    ...
    const merkleProofValidator2 = rewardsMerkleTree.getHexProof(
      leafsRewards[1]
    );
    const merkleProofValidator1 = rewardsMerkleTree.getHexProof(
      leafsRewards[0]
    );
    ...
    // Check claimRewards
    await expect(
      dappnodeSmoothingPool.claimRewards(
        validator1.address,
    ...
    await expect(
      ...
      .to.emit(dappnodeSmoothingPool, "EtherReceived")
      .withArgs(deployer.address, ethers.parseEther("100"));

    // Claim rewards
    const balancePoolRecipient = await ethers.provider.getBalance(
      validator1.address
    );
    await expect(
      dappnodeSmoothingPool
    ...
    const balancePoolRecipientAfter = await ethers.provider.getBalance(
      validator1.address
    );
    expect(balancePoolRecipient + availableBalanceValidator1).to.be.equal(
      balancePoolRecipientAfter
    );

    // Validator 2 delegate rewards
    expect(
      await dappnodeSmoothingPool.rewardRecipient(validator2.address)
    ).to.be.equal(ethers.ZeroAddress);

    await expect(
      dappnodeSmoothingPool
        .connect(validator2)
        .setRewardRecipient(rewardRecipientValidator2.address)
    )
      .to.emit(dappnodeSmoothingPool, "SetRewardRecipient")
      .withArgs(validator2.address, rewardRecipientValidator2.address);
    expect(
      await dappnodeSmoothingPool.rewardRecipient(validator2.address)
    ).to.be.equal(rewardRecipientValidator2.address);

    // Claim rewards
    const balancePoolRecipient2 = await ethers.provider.getBalance(
    ...
    const balancePoolRecipientAfter2 = await ethers.provider.getBalance(
      rewardRecipientValidator2.address
    );
    expect(balancePoolRecipient2 + availableBalanceValidator2).to.be.equal(
      balancePoolRecipientAfter2
    );
